...
const _setSearch = () => {
  ...
  const loadNodes = function (nodesDict) {
    const nodesArray = _.values(nodesDict).filter(node => node.isUnknown !== true && node.isAggregated !== true && FACT_SHEET_NODE_TYPE_WHITELIST.indexOf(node.type) != -1);
  ...
  const parseNode = (node) => {
    return {
      label: node.name.toLowerCase(), value: JSON.stringify({
        id: node.id, name: node.name.toLowerCase(), columnName: node.type.toLowerCase()
      })
    };
  };
