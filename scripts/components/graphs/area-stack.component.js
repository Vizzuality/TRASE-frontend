...
import { area as d3_area, stack as d3_stack } from 'd3-shape';
...
export default class {
  ...
  _setupData() {
    ...
    this.options = {
      width: this.el.clientWidth - 100, height: 380//this.el.clientHeight
    };
  ...
  _build() {
    ...
    // scale domains
    x.domain(d3_extent(this.data.includedYears, function (y) {
      return new Date(y, 0);
    }));
    ...
    this.data.includedYears.forEach((year, yearIndex) => {
      ...
      if (yearStackedValue > maxYearValue) {
        maxYearValue = yearStackedValue;
      }
    });
